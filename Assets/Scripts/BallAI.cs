...
public class BallAI : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        RacketAI = GameObject.Find("RacketDroite1");
    }

    void Update()
    {
        if(transform.position.y > RacketAI.transform.position.y)
        {
            RacketAI.GetComponent<ControllerDroitAI>().MoveUp();
        }

        if (transform.position.y < RacketAI.transform.position.y)
        {
            RacketAI.GetComponent<ControllerDroitAI>().MoveDown();
        }
